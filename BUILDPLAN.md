...
- **12.5 Image SEO decorator (filename label, alt text, hyperlink wrap)**
  - Goal: every image leaving Content Forge in HTML or Markdown is labeled, accessible, and (where a source URL exists) clickable. Closes the gap surfaced by the audit on 2026-05-13: alt-text presence is checked but never derived; filenames are sanitized but never validated as descriptive; images are never wrapped in anchors. Implementation lives in the SEO pipeline because it is fundamentally a render-time SEO and accessibility concern; the upload-side filename normalization that this decorator depends on lands in Phase 13 (Feature 11).
  - **12.5a Image decorator module**
    - New module `ContentForge.ContentGeneration.ImageDecorator` at `lib/content_forge/content_generation/image_decorator.ex`. Public function `decorate/2` takes a draft body (Markdown or HTML) plus a context map (the product, the list of ProductAssets the draft references, and the destination format `:markdown | :html | :platform_payload`) and returns the rewritten body.
    - For each image reference the decorator resolves the matching ProductAsset, then enforces three rules in order. First, if the image already has author-supplied alt text it is preserved. Second, if alt is missing, the decorator derives fallback alt text by humanizing the asset's publish filename slug (replace dashes and underscores with spaces, sentence-case the result). Third, if the asset has no publish filename or the publish filename is on the generic-pattern denylist, the decorator does not emit fallback alt — it leaves the image alt empty and records the asset id on a per-draft "undecorated images" list returned alongside the body.
    - Hyperlink wrapping rule: if the destination format is `:markdown` or `:html` AND the ProductAsset has a non-empty `source_url` field, the rendered image is wrapped in an anchor pointing at `source_url` with `rel="noopener"` and a `title` attribute matching the final alt text. For `:platform_payload` destinations the wrapping is skipped because most social platforms reject anchored images.
    - Generic-filename denylist is a configurable regex list under `:content_forge, :image_decorator, :generic_patterns`, defaulting to patterns covering IMG_, DSC_, DSCN, photo, screenshot, untitled, image, and bare digit-only stems.
    - Tests cover: author alt preserved; alt derived from publish filename slug when missing; generic-pattern asset yields empty alt and an entry in the undecorated list; anchor wrapping applied when source_url present and destination is HTML; anchor wrapping skipped for platform payloads even with source_url; Markdown image syntax and HTML img tag both rewritten correctly; multiple images in one body handled independently.
  - **12.5b SEO check for decorated image coverage**
    - New checklist module `ContentForge.ContentGeneration.SeoChecklist.Checks.ImageDecorationCoverage` alongside the existing `ImageAltCoverage` check. Returns `:pass` if every image in the draft has non-empty alt after decoration, `:fail` with a note listing the offending ProductAsset ids if any image is still missing alt because the underlying asset is generic-pattern, and `:not_applicable` for drafts with no images.
    - Wired into the SEO checklist runner as one of the 28 points (the existing `ImageAltCoverage` check is kept and continues to validate author-supplied alt only — the new check is the post-decoration gate).
    - The existing publish gate (12.4) blocks approval when the decoration coverage check fails. The failure note in the API response and dashboard drawer surfaces the asset ids so an operator can jump to the asset detail page and supply a description, which produces a publish filename via Feature 11.
    - Tests cover: pass case with all images decorated; fail case listing asset ids; not-applicable for image-less drafts; publish gate refuses approval when this check is red.
  - **12.5c Integration into publishing paths**
    - Blog webhook publisher (`ContentForge.Jobs.BlogPublisher`) calls the decorator with `:markdown` destination before assembling the webhook payload. The decorated body is what gets persisted and shipped.
    - Multi-platform publisher (`ContentForge.Publishing.MultiPlatform`) calls the decorator with `:platform_payload` destination for each platform-specific body. Social platforms receive bare images with derived alt text where supported (Twitter alt text API, LinkedIn image description) and ignore anchor wrapping.
    - Dashboard preview LiveView calls the decorator with `:html` destination so the preview shows the same anchored images the published HTML will contain.
    - Tests cover: blog publisher round-trip with a multi-image draft producing decorated Markdown; multi-platform publisher passes derived alt to each platform's alt field; dashboard preview matches the blog webhook output for the same draft.
  - Phase exit criteria for 12.5: a published blog post containing two images — one with a descriptive ProductAsset and a source URL, one with only a description — renders the first as an anchored image with derived alt, the second as a bare image with derived alt, and a third image with a generic-pattern filename causes the draft to fail the decoration coverage check and block approval until fixed.
...
- **13.6 Publish filename and source URL on ProductAsset**
  - Goal: give every asset a descriptive label that the Phase 12.5 image decorator can read for alt text and URL paths, plus an optional source URL that the decorator can use to wrap images in anchors. This is the upload-side half of the image SEO work; without it, the 12.5 decorator can only enforce "alt is missing" rather than supplying a sensible fallback.
  - **13.6a Schema extension and derivation rules**
    - Migration adds two columns to `product_assets`: `publish_filename` (string, nullable) and `source_url` (string, nullable, optional anchor target for the decorator). No index changes.
    - `ContentForge.ProductAssets.derive_publish_filename/1` is the single source of truth for how a publish filename is produced from an asset. Priority order: (1) any non-empty value already on `publish_filename`; (2) a slugified version of the asset's description when it is short enough to be a filename (configurable cap, default 80 characters); (3) the asset's tags joined with dashes; (4) a slugified version of the original filename only when it does not match the generic-pattern denylist (the same denylist used by the 12.5 decorator, loaded from the shared config key); otherwise nil. Nil means "needs description" and the dashboard surfaces that state.
    - On asset insert and on tag/description update, the context module calls `derive_publish_filename/1` and writes the result. Manual edits to `publish_filename` via the dashboard short-circuit derivation — once an operator has set a value explicitly, automatic re-derivation stops touching it.
    - Tests cover: each priority branch of the derivation produces the expected slug; generic original filename without any other input yields nil; explicit operator-set value is preserved across subsequent description updates; description longer than the cap falls through to the next priority instead of producing a giant filename.
  - **13.6b Dashboard editing and warning state**
    - Asset detail drawer on the product page gains two editable fields: "Publish filename" (text input showing the current value, with a humanized alt-text preview rendered live below the input) and "Source URL" (URL input with basic format validation). Editing either field saves on blur and refreshes the preview.
    - Assets where `publish_filename` is nil render with a warning badge "Needs description" in the asset grid. The bundle and draft-attachment UIs refuse to attach such an asset to a published draft, surfacing the same warning inline.
    - REST API extension: the existing asset detail endpoint includes `publish_filename`, `source_url`, and a derived `humanized_alt` field. The asset update endpoint accepts both new fields with the same validation as the dashboard form.
    - Tests cover: live preview updates correctly when the publish filename changes; warning badge is shown for assets without a publish filename; bundle attach blocks an asset in needs-description state with a clear error; API round-trip preserves both new fields.
  - Phase 13.6 exit criteria: every existing asset is either auto-populated with a publish filename via a backfill data migration or visibly flagged as needs-description in the dashboard; the Phase 12.5 decorator can resolve alt text for every asset that is allowed into a published draft.
...
- **16.3c Light writes on existing schemas: create_asset_bundle + add_tag_to_asset** ✅ Shipped `770dc2c`.
...
- **16.4a Confirmation envelope infra** ✅ Shipped `c30bd23`.
  ...
  - Ship `ContentForge.OpenClawTools.Confirmation` context module at `lib/content_forge/open_claw_tools/confirmation.ex` with two public functions.
    - `request(tool_name, ctx, params, preview)` derives `params_hash` by excluding the reserved `confirm` key (if present), encoding the remaining params with deterministic key ordering, hashing with SHA-256, and hex-encoding. It looks up a live pending row for the same `(session_id, tool_name, params_hash)` and returns its phrase + expiry unchanged if found (idempotent ask). Otherwise it generates a new echo phrase (random three words from the curated wordlist), inserts the row, and returns `{:ok, %{echo_phrase, expires_at, preview}}`. Missing `session_id` on ctx returns `{:error, :missing_session}`. Expiry defaults to `:content_forge, :open_claw_tools, :confirmation_expiry_seconds` (default 300).
    - `confirm(tool_name, ctx, params, echo_phrase)` looks up the pending row by `(session_id, echo_phrase)` excluding consumed rows. Row found + matching `tool_name` + matching `params_hash` + not expired: the row is marked consumed (single atomic update guarded by `consumed_at IS NULL` to handle race) and the function returns `:ok`. Row found but wrong `tool_name` or wrong `params_hash`: returns `{:error, :confirmation_mismatch}` without consuming. Row found but expired: `{:error, :confirmation_expired}`. Row not found: `{:error, :confirmation_not_found}`. Race where another concurrent confirm won the consume: `{:error, :confirmation_not_found}` (fail safely rather than executing a heavy action twice).
  - Wordlist: ship `priv/open_claw_tools/confirmation_words.txt` with ~120 short unambiguous words (single-syllable nouns + colors + places) across three buckets so `"adjective-noun-place"` shapes stay phonetic. Selection is uniform-random per bucket with `:crypto.strong_rand_bytes/1`. Collisions within an active window are retried once; after one retry the function falls through to simple random across the whole list. Wordlist is plain text, one word per line, loaded at module attribute time.
  - Extend `ContentForgeWeb.OpenClawToolController` to pattern-match a new tool return shape: `{:ok, :confirmation_required, %{echo_phrase, expires_at, preview}}`. The controller serializes this to `200 {"status":"confirmation_required","echo_phrase":"...","expires_at":"...","preview":{...}}`. Existing `{:ok, map}` / `{:error, reason}` / `{:error, :unknown_tool}` clauses are unchanged; the new clause goes above them. Error mapping adds `:missing_session`, `:confirmation_mismatch`, `:confirmation_expired`, `:confirmation_not_found` as tool-returnable reasons so the 422 path renders them as human strings.
  - No tool modules change in this slice. The infra is plumbing. 16.4b is the first consumer.
  - Tests (`test/content_forge/open_claw_tools/pending_confirmation_test.exs` for the schema, `test/content_forge/open_claw_tools/confirmation_test.exs` for the context, plus one controller test adding the `confirmation_required` response shape to the existing open_claw_tool_controller_test.exs):
    - Schema: inclusion + required-field checks; partial-unique index prevents two live rows with the same `(session_id, echo_phrase)` while permitting reuse after consumption; composite lookup index covers the idempotent-request path.
    - `Confirmation.request/4`: happy path returns phrase + expiry + preview; idempotent repeat with same `(session, tool, params_hash)` returns the same phrase; different `params_hash` mints a fresh phrase; missing `session_id` returns `:missing_session` without inserting; wordlist generator produces deterministic shape (three dash-separated lowercase words).
    - `Confirmation.confirm/4`: happy path marks consumed and returns `:ok`; wrong tool_name = `:confirmation_mismatch`; wrong params_hash = `:confirmation_mismatch`; missing row = `:confirmation_not_found`; consumed row = `:confirmation_not_found`; expired row = `:confirmation_expired`; race where two confirmations land simultaneously results in exactly one `:ok` and one `:confirmation_not_found` (covered with a concurrent task test that uses `Ecto.Adapters.SQL.Sandbox` checkout or `:shared` mode appropriately).
    - Controller: new clause serializes the `confirmation_required` triple end-to-end; echoes the tool-provided preview through the response body.

- **16.4b approve_draft (first consumer of the confirmation envelope)** ✅ Shipped `3cd5f9c`.
  - Blocks: none. Blocked by: 16.4a.
  - Ship `ContentForge.OpenClawTools.ApproveDraft` at `lib/content_forge/open_claw_tools/approve_draft.ex`. Params: `"draft_id"` (required UUID), `"override_reason"` (optional string, required when the 12.4 publish gate would otherwise block), `"product"` (optional, resolver), `"confirm"` (optional echo phrase).
  - Flow: resolve product, resolve draft scoped to product (else `:not_found`), `Authorization.require(ctx, :owner)`. If `confirm` absent call `Confirmation.request/4` with a `preview` map carrying `%{summary: "...", draft_id, platform, content_type, angle, snippet, publish_gate: :passes | :blocks, required_override: bool, override_reason_present: bool}`. If `confirm` present call `Confirmation.confirm/4`; on `:ok` execute the existing 12.4 approval flow: if gate blocks and no `override_reason`, return `{:error, :publish_gate_blocks}`; if gate blocks and `override_reason` given, take the override path; on pass, take the plain approve path. Return `%{draft_id, status, approved_at, approved_via_override, override_reason}`.
  - Tool returns `:forbidden` for non-owner roles, `:not_found` for unknown draft or cross-product draft, the standard confirmation error reasons, and `:publish_gate_blocks` for a clean gate refusal (the agent reads the preview back to the user to explain why).
...
- **16.4d generate_drafts_from_bundle** ✅ Shipped `7df57c6`.
  ...
  - First call (no confirm): return `Confirmation.request/4` with `preview: %{summary, bundle_id, asset_count, estimated_cost_cents, remaining_budget_cents, would_exceed_budget: bool}`. The preview MUST carry the budget numbers so the agent can read them to the user before asking for the phrase. If `would_exceed_budget` is true the preview includes a short `warning` string the agent is expected to pass on verbatim.
  - Second call (confirm matches): enqueue `ContentForge.Jobs.AssetBundleDraftGenerator` with the resolved bundle + product. Return `%{enqueued: true, bundle_id, estimated_cost_cents, remaining_budget_cents, job_args}`.
