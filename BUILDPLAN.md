...
- **11.2L Bulk variant generation via LLM clients** (supersedes the paused 11.2 caller — OpenClaw runs locally but is turn-oriented, not batch-oriented; Anthropic + Gemini are already shipped, cheaper, and the native shape for non-conversational bulk content)
...
- **14.2c Real conversational reply via OpenClaw gateway** (unblocked 2026-04-23 — OpenClaw gateway found running on `localhost:18789`, agent-turn surface verified with a live ping/pong)
