...
Details per item:
- 1a: mix phx.new content_forge --database postgres, add oban + exaws_s3 + req + zyzyva_telemetry deps, configure Oban in application.ex, create Product schema (name, repo_url, site_url, publishing_targets as jsonb map of platform configs with enabled/cadence per platform). Run mix ecto.create && mix ecto.migrate.
