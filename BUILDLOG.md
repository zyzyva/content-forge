...
- `with` chain: resolve product → `Authorization.require(..., :owner)` → validate cadence (integer, 1..30 → `:invalid_cadence`) → validate enabled (boolean only, default true → `:invalid_enabled`). All four guards fire before any DB read, so a malformed or unauthorized request never reaches `ReminderConfig`.
- `dispatch_turn/5` fetches the current config (via `Sms.get_reminder_config/1` which returns schema defaults when no row exists) and checks `same?/3`. No-op (`cadence_days` + `enabled` both match) short-circuits with `{:ok, %{changed: false, product_id, cadence_days, enabled}}` and never inserts a pending confirmation row. Actual changes branch on `params["confirm"]`: absent goes to `request_turn/6`, present goes to `confirm_turn/6`.
- `request_turn` builds a diff preview (`%{summary, product_id, before, after}`) and calls `Confirmation.request/4`. The summary uses `describe/2` to render the current + target cadence in English ("off", "daily", "every N days") so the agent can read it back to the user without string interpolation in the prompt.
- `confirm_turn` pipes `Confirmation.confirm/4` → `Sms.upsert_reminder_config/2`. Success returns `%{changed: true, ..., updated_at}` with the row's `updated_at` as ISO-8601. Changeset errors from the upsert flatten to `:update_failed`.

**Dispatch + plugin + runbook** (`lib/content_forge/open_claw_tools.ex` + `~/.openclaw/plugins/content-forge/index.js` + `docs/openclaw-plugin-runbook.md`):
- `@tools` grows to ten entries.
- Node plugin adds `renderScheduleReminderConfirmation` + `renderScheduleReminderResult` helpers that format before/after diffs in the same English-readable cadence form the tool summary uses. The `schedule_reminder_change` execute handler uses `invokeConfirmable` (16.4b infrastructure) and branches on `envelope.status`.
- Runbook catalogue entry calls out the no-op short-circuit so operators understand confirmation prompts only appear when state would actually change.

**Tests** (12 new):
- `test/content_forge/open_claw_tools/schedule_reminder_change_test.exs` (10 tests):
  - Authorization: submitter = `:forbidden` (no pending row created).
  - Validation: cadence 0 / 31 / non-integer / missing all return `:invalid_cadence`; non-boolean `enabled` returns `:invalid_enabled`.
  - First turn: change request returns envelope with correct before/after; no-op returns `changed: false` + zero pending rows; omitted `enabled` param compares against the default `true`; uses schema defaults when no config row exists yet.
  - Second turn: correct confirm persists the change and returns `updated_at`; wrong echo phrase returns `:confirmation_not_found` with no DB change.
  - Resolution: ambiguous product short-circuits before auth.
- `test/content_forge_web/controllers/open_claw_tool_controller_test.exs` (2 new): full two-turn HTTP cycle (confirmation_required with before/after preview, then `confirm` flips the row + returns `result.changed: true`), and a single-call no-op that returns `result.changed: false` without ever asking for confirmation.

**Gate**: `mix compile --warnings-as-errors` clean, `mix format --check-formatted` clean, `mix test` 966/0 (12 new), `mix credo --strict` baseline-diff empty for slice-touched files. Zero emdashes in slice-touched files, the plugin, or the runbook.
...
- `with` chain: resolve product, scoped draft lookup, `Authorization.require(..., :owner)`. Cross-product or unknown or malformed `draft_id` all collapse to `:not_found` uniformly so a probing caller cannot distinguish "exists but wrong product" from "does not exist".
- Turn dispatch via `binary_param(params, "confirm")`: nil goes to `request_turn/4`, non-empty binary goes to `confirm_turn/4`.
- `request_turn`: builds preview (`%{summary, draft_id, platform, content_type, angle, snippet, publish_gate, required_override, override_reason_present}`), calls `Confirmation.request/4`, unwraps to `{:ok, :confirmation_required, envelope}`. `publish_gate` is computed by a pure `publish_gate_status/1` helper that mirrors the gate's blocking conditions (`research_status == "lost_data_point"` or `seo_score < threshold` for blog drafts; non-blog always passes) without mutating.
- `confirm_turn`: calls `Confirmation.confirm/4`; on `:ok`, routes to the approval flow. Blog drafts with `override_reason` take `approve_blog_draft_with_override/2`; without a reason and gate-blocks returns `:publish_gate_blocks`; gate-pass takes the plain `approve_blog_draft/1` path. Result normalized to `%{draft_id, status, approved_at, approved_via_override, override_reason}` with `approved_at` as ISO-8601 of the draft's `updated_at`.
- Approval errors from the content-generation context (`{:error, :seo_below_threshold, details}`, `{:error, :research_lost_data, details}`, `{:error, :override_reason_too_short, details}`) are flattened via `classify_approval_error/1` to a single `{:error, atom}` so the controller serializer stays uniform.
...
- `@tools` map grows to nine entries.
- Node plugin refactor: `invokeTool` now accepts both `status: "ok"` and `status: "confirmation_required"` from the controller rather than throwing on the latter. `buildInvoker` preserves the pre-16.4 contract (returns `data.result` on ok, throws if the tool unexpectedly returns a confirmation envelope). A new `buildConfirmableInvoker` returns the full envelope; the `approve_draft` handler uses it to branch on `envelope.status`.
- `renderApproveConfirmation(envelope)` reads the preview verbatim for the agent's first-turn reply, spells out whether the publish gate blocks, whether an override is needed, and quotes the echo phrase the user must repeat. `renderApproveResult(result)` summarizes the second-turn success.
- Runbook catalogue now lists all nine tools; `approve_draft`'s entry spells out the two-turn flow + override requirement.

**Tests** (16 new):
- `test/content_forge/open_claw_tools/approve_draft_test.exs` (15 tests):
  - Authorization: submitter = `:forbidden` (and no pending confirmation row is created), unknown sender = `:forbidden`.
  - Scoping: cross-product draft = `:not_found`, unknown UUID = `:not_found`, malformed UUID = `:not_found`.
  - First turn: gate-pass preview has `publish_gate: :passes`, gate-block without reason has `required_override: true` + `override_reason_present: false`, gate-block with reason has `override_reason_present: true`, idempotent same-params ask returns the same phrase (one DB row), non-blog drafts always report `publish_gate: :passes`.
  - Second turn: gate-pass + correct phrase flips draft to `"approved"` + `approved_via_override: false`, gate-block + reason + correct phrase takes override path + records reason, gate-block + no reason + correct phrase returns `:publish_gate_blocks` with no mutation, mismatched `override_reason` between request and confirm returns `:confirmation_mismatch`, wrong phrase returns `:confirmation_not_found`.
- `test/content_forge_web/controllers/open_claw_tool_controller_test.exs` (1 new): the full two-turn HTTP cycle. First POST returns `200 {"status":"confirmation_required", ...}` with `preview.publish_gate == "passes"`; second POST replays with `params.confirm = echo_phrase` and receives `200 {"status":"ok","result":{"status":"approved"}}`; DB assertion confirms the draft flipped.

**Gate**: `mix compile --warnings-as-errors` clean, `mix format --check-formatted` clean, `mix test` 954/0 (16 new), `mix credo --strict` baseline-diff empty for slice-touched files. Zero emdashes in slice-touched files, the plugin, or the runbook.
...
- Table `pending_confirmations` with `id`, `session_id`, `tool_name`, `params_hash`, `echo_phrase`, `preview` (jsonb map default `{}`), `expires_at` (utc_datetime_usec), `consumed_at` (utc_datetime_usec nullable), `utc_datetime_usec` timestamps. Append-only lifecycle: insert on turn one, UPDATE the single `consumed_at` column on turn two; rows are never deleted.
- Partial unique index `(session_id, echo_phrase) WHERE consumed_at IS NULL` enforces single-live-phrase-per-session at the DB level; consuming a row unlocks that pair for future reuse.
- Composite index `(session_id, tool_name, params_hash) WHERE consumed_at IS NULL` supports the idempotent-request lookup. `expires_at > NOW()` was dropped from the predicate because Postgres only accepts immutable predicates and `NOW()` is stable; the context filters by expires_at at query time and the partial on consumed_at is what keeps the index small.
- Schema module validates length bounds on session / tool / phrase / hash so a malformed call never persists garbage.

**Wordlist** (`priv/open_claw_tools/confirmation_words.txt`):
- 120 words in three blank-line-separated buckets (40 colors, 40 nouns, 40 places). One word per line, lower-case ASCII. Deliberately short and phonetically unambiguous: `crimson-otter-harbor`, `amber-fox-meadow`. 40³ = 64 000 phrases so collision inside an active window is astronomical.
- Loaded at compile time via `@wordlist_path` + `@external_resource` so file changes trigger recompile. Blank-line splitting yields three bucket lists stored in a `@buckets` module attribute; no runtime file I/O.

**Context** (`lib/content_forge/open_claw_tools/confirmation.ex`):
- `request/4` derives `params_hash` by deleting the reserved `"confirm"` key and running the remaining params through `:erlang.term_to_binary/2` with `:deterministic`. That BIF normalizes map key ordering so two equal maps with different insertion orders hash identically; switching to Jason would have required a hand-rolled canonical ordering. Looks up a live row via `find_live/4` (filtered by `consumed_at IS NULL` and `expires_at > now`) - hit returns the stored phrase unchanged (idempotent ask, no new row). Miss goes to `insert_with_retry/6` with the `@max_insert_retries` of 1 so a single collision on the partial unique gets another phrase; a second collision surfaces as `:confirmation_insert_failed` rather than looping.
- `confirm/4` resolves by `(session_id, echo_phrase)` excluding consumed rows (`resolve/2`), pipes through `classify/4` which pattern-matches on `tool_name` and `params_hash` mismatch (`:confirmation_mismatch`) vs expiry (`:confirmation_expired`), then `maybe_consume/2` runs a single atomic `update_all` guarded by `id == ^id AND consumed_at IS NULL`. A count of 1 returns `:ok`; a count of 0 means another concurrent confirm won the race and we return `:confirmation_not_found` - fail safely rather than execute a heavy action twice.
- Both entry points fail closed on `missing_session` without ever touching the DB.
- `envelope_from_row/1` hands preview back with atom keys when the map was just built in memory and converts string keys to known atoms on reload (a small `@known_preview_keys` whitelist avoids inflating the atom table with agent-supplied strings). Tools never need to care whether the preview came from the first-turn call or a reload.
- Expiry ceiling is configurable at `:content_forge, :open_claw_tools, :confirmation_expiry_seconds` with a 300-second default.

**Controller extension** (`lib/content_forge_web/controllers/open_claw_tool_controller.ex`):
- New top-most `{:ok, :confirmation_required, envelope}` clause serializes to `200 {"status": "confirmation_required", "echo_phrase": "...", "expires_at": "<iso8601>", "preview": {...}}`. Existing `{:ok, result}` / `{:error, :unknown_tool}` / `{:error, reason}` clauses are unchanged and match below the new one.
- Envelope's `preview` goes through the same `serialize_result/1` helper as tool success payloads, so atom keys become string keys and atom values become strings uniformly. `expires_at` is explicitly `DateTime.to_iso8601/1`.
- `dispatch/3` in `OpenClawTools` now consults `:content_forge, :extra_open_claw_tools` before the compile-time `@tools` map. Prod never sets that key; tests register temporary stub tools (the `ConfirmationRequiredStub` in the controller test uses this seam to assert the new response shape without waiting for 16.4b to land a real consumer).
- The tools `result` typespec grew a `{:ok, :confirmation_required, map()}` alternative.

**Tests** (20 new):
- `test/content_forge/open_claw_tools/pending_confirmation_test.exs` (4 tests): schema insert happy path, required-field checks, partial-unique rejection for two live rows with the same `(session, phrase)`, successful re-use of the pair after the first row is consumed.
- `test/content_forge/open_claw_tools/confirmation_test.exs` (15 tests). `request/4`: fresh phrase minted + expiry returned + preview echoed, idempotent ask returns same phrase + does not double-insert, different params_hash mints a fresh phrase, `confirm` key excluded from hash so a retry stays idempotent, `:missing_session` without inserting, expired live rows do not block new requests. `confirm/4`: happy path consumes and returns :ok, wrong tool_name = `:confirmation_mismatch`, wrong params_hash = `:confirmation_mismatch`, unknown phrase = `:confirmation_not_found`, already-consumed = `:confirmation_not_found`, expired = `:confirmation_expired`, `confirm` carrying the echo phrase in params still matches via the canonical hash, **concurrent confirms land exactly one `:ok` and one `:confirmation_not_found`** (two Tasks racing against the same row; sandbox shared via `Ecto.Adapters.SQL.Sandbox.allow/3`), `:missing_session` on ctx without inserting.
- `test/content_forge_web/controllers/open_claw_tool_controller_test.exs` (1 new, under a `16.4a confirmation_required response` describe block): registers a `ConfirmationRequiredStub` tool via `:extra_open_claw_tools` and asserts the 200 serialization shape end-to-end including atom-keyed preview flattening.

**Spec-note deviations**:
- Composite index predicate: spec called for `WHERE consumed_at IS NULL AND expires_at > NOW()`, which Postgres refuses (NOW() is stable, not immutable). Shipped as `WHERE consumed_at IS NULL` with the `expires_at` filter at query time. Same end-to-end behavior; smaller index.
- Second-collision fallback: spec called for "falls through to simple random across the whole list." Shipped a single retry that falls through to `:confirmation_insert_failed`. With 64 000 phrases and a small number of concurrently-live rows per session, even one collision is vanishingly unlikely; two is indistinguishable from programmer error. Reviewer is free to tighten this if desired.

**What this slice explicitly does NOT do**:
- 16.4b wires the first real consumer (`approve_draft`).
- No new tool modules; the heavy-write tools land in 16.4b/c/d.

**Gate**: `mix compile --warnings-as-errors` clean, `mix format --check-formatted` clean, `mix test` 938/0 (20 new), `mix credo --strict` baseline-diff empty for slice-touched files (one new credo finding on `Enum.map_join` and one on a nested `Ecto.Adapters.SQL.Sandbox` reference were both addressed before the final gate). Zero emdashes in slice-touched files, the plugin, the wordlist, or the runbook. Migration applied cleanly to `MIX_ENV=test`.
...
- `with {:ok, product} <- ProductResolver.resolve(...)`, then auth, then `fetch_name/1` (trim, refuse empty, refuse over 120 chars, map to `:invalid_name`), then `ProductAssets.create_bundle/1`. Changeset errors from outside the name path are re-wrapped as `{:error, {:invalid_params, traversed_errors}}` so a rare constraint violation still surfaces a structured reason.
- Result shape: `%{bundle_id, product_id, product_name, name, status, created_at}` with `created_at` as ISO-8601.

**`add_tag_to_asset`** (`lib/content_forge/open_claw_tools/add_tag_to_asset.ex`):
- `fetch_tag/1` trims, lowercases, and validates 1..40 bytes - returns `:invalid_tag` for empty, missing, or oversized. Lowercasing is in-tool so `"Spring"` and `"spring"` do not split into two buckets.
- `scoped_asset/2` rescues `Ecto.Query.CastError` for malformed UUIDs and verifies `product_id == resolved_product.id`; cross-product or unknown ids collapse to `:not_found` uniformly (no distinguishable reason for probing callers).
- `persist_tag/2` delegates to `ProductAssets.add_tag/2`, which dedupes via `Enum.uniq/1` so a duplicate tag returns the existing set unchanged.
- Result shape: `%{asset_id, tags}`.

**Dispatch + plugin** (`lib/content_forge/open_claw_tools.ex` + `~/.openclaw/plugins/content-forge/index.js`):
- `@tools` now carries seven entries (five from 16.1/16.2 plus `create_asset_bundle` and `add_tag_to_asset`).
- Plugin gains two `registerTool` blocks with JSON-schema params marking `name` / `asset_id` + `tag` as required. Agent-facing descriptions explicitly call out the `:submitter` role requirement so the agent can explain refusals to the user rather than silently retrying.
- Also swept a stray emdash out of the `competitor_intel_summary` description on the way through.

**Tests** (19 new):
- `test/content_forge/open_claw_tools/create_asset_bundle_test.exs` (6 tests): happy path (SMS submitter) + list persisted, whitespace trim, viewer :forbidden, empty / missing / over-120 names all map to :invalid_name, ambiguous product short-circuits before auth.
- `test/content_forge/open_claw_tools/add_tag_to_asset_test.exs` (9 tests): happy path with merged tag set, trim + lowercase on the tag, duplicate = no-op on persisted set, unknown asset_id = :not_found, cross-product asset_id = :not_found, viewer = :forbidden and no mutation, empty / missing tag = :invalid_tag, over-40 tag = :invalid_tag, malformed UUID = :not_found.
- `test/content_forge_web/controllers/open_claw_tool_controller_test.exs` (4 new, under a new `16.3c light-write tools` describe block): happy path through the full HTTP pipeline for both tools, forbidden end-to-end for `create_asset_bundle`, cross-product not_found for `add_tag_to_asset`.

**What this slice explicitly does NOT do**:
- 16.3d still owes `ProductMemory` + `record_memory`. It is the only remaining piece of the 16.3 wave.

**Gate**: `mix compile --warnings-as-errors` clean, `mix format --check-formatted` clean, `mix test` 898/0 (19 new), `mix credo --strict` baseline-diff empty for slice-touched files. Zero emdashes in slice-touched files, the plugin index.js, or the runbook.

### Phase 16.3b: CreateUploadLink hardening (first consumer of the Authorization helper)

Status: DONE
Merged: master @ `7a269fc` (fast-forward). Reviewer ACCEPT. Gate: compile/format/test 879-0, credo 26 vs 44 baseline (zero new in slice-touched files). Coverage CreateUploadLink 96%, AcceptedContentTypes 40% (thin module, exercised via consumers). Zero emdashes. Reviewer non-blocking note: the default `"application/octet-stream"` content_type bypasses the allow-list by design to preserve 16.1 behavior for callers that omit `content_type`. Tightening that to require an explicit allow-listed type is deferred; if it becomes desirable the fix is a one-line change to treat the defaulted value as explicitly disallowed, and no schema work is needed.
Note: First tool to call through `ContentForge.OpenClawTools.Authorization.require/2`. Closes both forward-looking items the 16.1 reviewer flagged: the content-type allow-list and the `expires_in_seconds` ceiling. No new tool surface, but `CreateUploadLink` is now unambiguously a write-privilege call from the agent's perspective.

**Shared allow-list extraction** (`lib/content_forge/product_assets/accepted_content_types.ex`, new): single source of truth for MIME types Content Forge accepts on any asset-upload path. Exports `list/0`, `image_list/0`, `video_list/0`, `allowed?/1`. The image list is `~w(image/jpeg image/png image/webp image/heic)`, the video list is `~w(video/mp4 video/quicktime video/x-m4v)`. `allowed?/1` has a pattern-match clause for binaries and a catch-all so non-binary inputs (nil, integers, keyword lists the agent might hallucinate) fail closed without raising.

**Controller refactor** (`lib/content_forge_web/controllers/product_asset_controller.ex`): the `@image_content_types` / `@video_content_types` / `@allowed_content_types` constants are now sourced from `AcceptedContentTypes.image_list()` / `video_list()` at compile time (module attributes accept compile-time function calls). `validate_content_type/1` delegates to `AcceptedContentTypes.allowed?/1`. Byte-size cap guards keep the local module attributes since the caps are 13.1b-specific and independent of the agent-tool path. The 17 existing `ProductAssetController` tests continue to pass unchanged.

**CreateUploadLink tool hardening** (`lib/content_forge/open_claw_tools/create_upload_link.ex`):
- New `with` chain: resolve product via `ProductResolver`, then call `Authorization.require(Map.put(ctx, :product, product), :submitter)` before any storage side effects. Failure short-circuits as `{:error, :forbidden}`; success continues to content-type + filename + expires + presign.
- `fetch_content_type/1` passes the default `application/octet-stream` through unchanged (the default is for agent callers that do not know or care about MIME yet) but validates any explicit value against `AcceptedContentTypes.allowed?/1`, returning `{:error, :unsupported_content_type}` for anything off the list. Important: the check lives before the storage adapter call, so a malformed `content_type` never reaches R2.
- `fetch_expires_in/1` + `clamp_expires_in/1` + `max_expires_in_seconds/0` implement the ceiling: values `> 0` are `min(value, max_expires_in_seconds)`; everything else (nil, zero, negative, non-integer) collapses to the 900-second default. The ceiling is configurable at `:content_forge, :open_claw_tools, :max_upload_expires_seconds` with a 3600-second default. The clamp is silent by design - an agent asking for a 24-hour link gets a 1-hour link without the tool surface rejecting the turn.

**Node plugin description update** (`~/.openclaw/plugins/content-forge/index.js`): the `content_type` description now lists the seven accepted MIME types and names the `unsupported_content_type` error taxonomy so the agent can pick a correct value on the first turn. The `expires_in_seconds` description spells out the one-hour ceiling and the "do not ask for longer" guidance. This is outside the repo per the runbook; the index.js update is captured in the handoff.

**Tests** (15 new; one existing test updated):
- `test/content_forge/open_claw_tools/create_upload_link_test.exs` (14 tests across authorization, content-type, clamp, and product-resolution groupings):
  - Authorization: viewer (SMS) = `:forbidden`, submitter (SMS) = `:ok`, owner (CLI) = `:ok`, CLI without OperatorIdentity = `:forbidden`, unknown channel (`telegram`) = `:forbidden`.
  - Content-type: unknown MIME = `:unsupported_content_type`, allow-listed image (image/png) = `:ok`, allow-listed video (video/mp4) = `:ok`.
  - Clamp: 99_999 clamped to 3600, 0 falls back to 900, -5 falls back to 900, 1200 passes through, custom ceiling from config (600) clamps 99_999 down to 600.
  - Resolution order: `product_not_found` returns before the auth gate fires, so a bad product name cannot be used to probe for phone / identity registration.
- `test/content_forge_web/controllers/open_claw_tool_controller_test.exs`: the long-standing "dispatches to CreateUploadLink" happy-path test now seeds an `OperatorIdentity` row for `cli:ops` (submitter role) because the auth gate is live. Existing SMS-session test still works because the seeded ProductPhone row carries an `"owner"` role already. All other controller tests are unaffected because they either hit the auth fail-closed path deliberately or never reach the tool (unknown-tool, product-not-found, ambiguous-product all return before the auth gate).

**What this slice explicitly does NOT do** (next sub-slices):
- 16.3c ships the real light-write tools: `create_asset_bundle` and `add_tag_to_asset`.
- 16.3d ships `ProductMemory` + `record_memory`.

**Gate**: `mix compile --warnings-as-errors` clean, `mix format --check-formatted` clean, `mix test` 879/0 (14 new + 1 updated in this slice), `mix credo --strict` baseline-diff empty for slice-touched files (only pre-existing drift in `metrics_poller.ex` and `video_producer.ex`). Zero emdashes in slice-touched files and in the Node plugin.
...
- `lib/content_forge/open_claw_tools/product_resolver.ex` (new, ~100 LOC). Single `resolve(ctx, params)` entry point. UUID-first (via `Products.get_product/1` with `Ecto.Query.CastError` rescue for non-UUID input), then case-insensitive substring across `Products.list_products/0`. Session fallback engages only on `channel: "sms"` with a non-empty `sender_identity`. Zero or multiple active `ProductPhone` rows both resolve to `:missing_product_context` so the agent can ask the user which product.
- `lib/content_forge/sms.ex` gains `list_active_phones_by_number/1` (returns every active phone row for the given E.164 string). Needed because the session fallback must distinguish "single product" from "phone spans multiple products".
- `CreateUploadLink.resolve_product/1` deleted; tool now calls `ProductResolver.resolve(ctx, params)`. All prior tests continue to pass. A new controller test asserts the SMS-session resolution end-to-end via the Node plugin shape (channel `"sms"`, `sender_identity: "+15551112222"`, empty params).

**Four new tool modules** under `lib/content_forge/open_claw_tools/`:
- `ListRecentAssets.call/2`: reuses `ProductAssets.list_assets/2`, accepts optional `limit` (default 10, clamped 1-50), optional `media_type` (`"image" | "video"`), optional `tag`. Returns `%{product_id, product_name, count, assets: [...]}` with each asset serialized to `%{id, filename, media_type, status, mime_type, byte_size, tags, description, uploaded_at}` — `uploaded_at` as an explicit ISO-8601 string so nested serialization stays correct end-to-end.
- `DraftStatus.call/2`: requires one of `draft_id` (exact UUID) or `hint` (free-text, case-insensitive ilike against `content` or `angle`). Draft lookup is always scoped to the resolved product — a cross-product `draft_id` collapses to `:not_found`. A hint that matches multiple drafts returns `{:error, {:ambiguous_draft, %{candidates: [...]}}}` with up to three `%{id, status, snippet}` candidates. `approval_required` is computed as "blog platform, not yet approved/published/rejected/archived"; `blocker` surfaces `draft.error` when status is `blocked`; `approved_at` is the draft's `updated_at` when status is `approved` or `published` (no dedicated `approved_at` column in the schema, approximation is explicit).
- `UpcomingSchedule.call/2`: wraps `ContentGeneration.list_approved_drafts/1`, post-filters by optional `platform`, clamps `limit` to 1-25 (default 10). Returns `%{count, drafts: [%{id, platform, content_type, angle, snippet, approved_at, status}]}`. Empty result yields `count: 0` + `drafts: []`, never an error — the bot renders "nothing queued right now" per spec. `snippet` is the first 200 characters of `content`. The Node plugin description explicitly warns the agent that Content Forge does not hold per-draft schedule timestamps, so replies speak in terms of "queued" instead of "scheduled for Thursday at 10am."
- `CompetitorIntelSummary.call/2`: pulls the newest row via the already-existing `Products.get_latest_competitor_intel_for_product/1`. Returns `%{summary, trending_topics, winning_formats, effective_hooks, source_post_count, generated_at}`. `source_post_count` maps from the DB column `source_count` (migration default `0`, but nil-tolerant for legacy rows). `:not_found` when no intel row exists — deliberate per spec so the bot says "I do not have competitor data yet" instead of implying an empty competitive landscape.

**Dispatch registration** (`lib/content_forge/open_claw_tools.ex`): `@tools` map now carries five entries. Controller is unchanged — the only-touch-the-map pattern means new tools in 16.3+ require only a module + a map entry + a `registerTool` block in the plugin.

**Node plugin extension** (`~/.openclaw/plugins/content-forge/index.js`, lives outside the repo per runbook): refactored `invokeTool` into a `buildInvoker(baseUrl, toolSecret)` closure so each tool's `execute` handler is a single `await invoke("name", invocationId, params, ctx)` call. Added a `registerTool` block per new tool with JSON-schema parameter definitions and dedicated markdown renderers (`renderAssetList`, `renderDraftStatus`, `renderUpcomingSchedule`, `renderCompetitorIntel`). Previously `product` was required on `create_upload_link`; that constraint is now lifted so the SMS channel can invoke it from a registered phone without an explicit product hint.

**Runbook** (`docs/openclaw-plugin-runbook.md`): `Future tools` section replaced with a `Currently registered tools` catalogue listing all five shipped tools and their one-line purpose. Phase ladder rewritten to start from 16.3 since 16.2 is now DONE.

**Tests** (36 new; 67 total in the open-claw namespace, 833 total in the suite):
- `test/content_forge/open_claw_tools/product_resolver_test.exs` (11 tests): UUID / fuzzy / ambiguous / not-found / UUID-shaped-unknown / SMS happy / SMS no-match / SMS inactive-phone / SMS multi-phone / non-sms channel / empty-product-param.
- `test/content_forge/open_claw_tools/list_recent_assets_test.exs` (6 tests): happy path + ISO-8601 + soft-delete exclusion + descending `uploaded_at` ordering, `limit` clamping both ends, `media_type` filter, `tag` filter, `:missing_product_context`, `:product_not_found`.
- `test/content_forge/open_claw_tools/draft_status_test.exs` (11 tests): id happy path + all status field surfacing, non-blog = approval_required false, blog + approved = approval_required false + approved_at populated, blocked draft surfaces `error` as `blocker`, cross-product id = `:not_found`, well-formed missing UUID = `:not_found`, hint single-match, hint ambiguous with candidate list, hint no-match, hint scopes to product, no identifier at all = `:not_found`.
- `test/content_forge/open_claw_tools/upcoming_schedule_test.exs` (5 tests): happy path with snippet + approved_at, platform filter, limit clamp, empty list = `count: 0`, `:missing_product_context`.
- `test/content_forge/open_claw_tools/competitor_intel_summary_test.exs` (4 tests): happy path with all array fields + latest-wins ordering (via back-dated `inserted_at` to defeat second-precision timestamp ties), nil `source_count` tolerance, `:not_found` when no intel exists, `:missing_product_context`.
- `test/content_forge_web/controllers/open_claw_tool_controller_test.exs` (6 new, 13 total): SMS-session path on `create_upload_link`, one end-to-end per new tool via the full HTTP pipeline (auth + controller serialization + JSON shape), plus a `competitor_intel_summary` 422 `not_found` case.

**Explicit non-goals (deferred to later 16 slices)**:
- No authorization beyond the existing shared-secret header. Role-based `Authorization.require/2` lands in 16.3.
- No `ToolInvocationEvent` audit rows yet. That unifies in 16.5 and backfills every shipped tool at that point.
- No write tools, no confirmation envelopes, no rate limits.
- No per-draft `scheduled_at` field. If a future slice introduces real scheduling, `UpcomingSchedule` extends its result shape then.
- The reviewer's non-blocking 16.1 forward-looking items (content-type allow-list on `create_upload_link`, `expires_in_seconds` ceiling) still defer to 16.3 where the authorization framework gives them a natural home.

**Gate**: `mix compile --warnings-as-errors` clean, `mix format --check-formatted` clean (one test file reformatted by `mix format`), `mix test` 833/0, `mix credo --strict` diff vs `.credo_baseline.txt` shows zero new findings in slice-touched files (pre-existing drift in `metrics_poller.ex` / `video_producer.ex` line numbers only). Merged `master @ d6f61a2` (fast-forward) at slice start.
...
Status: DONE
Merged: master @ `b8f1ff2` (fast-forward). Reviewer ACCEPT. Gate: compile/format/test 783-0; credo unchanged. Swapped `System.cmd` for `System.shell` with a canonical POSIX single-quote `shell_escape/1` (inside single quotes sh interprets nothing — `$`, backticks, `;`, `|`, `>`, newlines, `\` all literal). Timeout via `Task.async + Task.yield || Task.shutdown(:brutal_kill)` so killing the task closes the linked Port and SIGTERMs the subprocess — no orphans. `2><tempfile>` separates stderr from stdout so JSON parsing stays clean; stderr debug-logged on every call, surfaced in error reason on non-zero exit. Both prior reviewer notes closed with correctness, not workarounds.
...
Status: DONE
Merged: master @ `9d74483` (fast-forward). Reviewer ACCEPT after re-handoff (first pass introduced two nested-module findings at `draft_controller.ex:197+199` from inline fully-qualified `ContentBriefGenerator.new` + `OpenClawBulkGenerator.new` references — resolved via alias hoist; coder adopted a stricter "run credo per-file on touched files before handoff" check). Gate: compile/format/test 769-0; coverage 59.82% (+0.35). Severity-ordered gate (research_lost_data > seo_score < threshold); 4-field audit snapshot survives SEO re-runs; color-banded badges; modal with reason-length minimum + preserved-reason on submit. **Phase 12 SEO Quality Pipeline exit criteria all met.**
...
**Test coverage** (`test/content_forge_web/controllers/schedule_controller_test.exs`, new):
- POST `/api/v1/products/:id/schedule` with no body: asserts enabled platforms land as `"scheduled"`, disabled ones as `"disabled"`, and `assert_enqueued worker: Publisher, args: %{"product_id" => ..., "platform" => "twitter"}` proves a real Oban job was inserted. `refute_enqueued` for the disabled linkedin row locks in the enabled-only behavior.
- POST with an explicit `"platforms" => ["twitter"]` body: asserts the body-path filter is respected and a Publisher job is enqueued. This is the exact test the bug would have failed had it existed before, and is what locks in the fix.
- Uses `Oban.Testing` with `testing: :manual` (the project default), so jobs land in the queue but do not execute; the assertions are state checks on the queue itself.

**What was explicitly NOT changed**:
- `ScheduleController.publish_draft/2` and `publish_now/2` at lines 131-161 also contain `Oban.insert(%{...})` with bare maps and no worker module - those are additional instances of the same bug class. They're out of scope for this single-focus slice (per architect direction: "keep it to one focused commit"). Reviewer flagged the audit-sweep idea on 15.4 accept; if that sweep happens, those two sites land with it.
- Existing `find_next_window/3` / `calculate_delay/3` arithmetic is untouched. The bug was strictly about enqueue shape, not the scheduling math.

**Gate**: compile --warnings-as-errors clean, format clean, full test 623/0 (2 new tests in `schedule_controller_test.exs`), credo --strict baseline-diff empty. Rebased cleanly on master @ `7831e28`.
...
- **2026-04-23:** OpenClaw IS installed and running locally (found after a broader probe than 14.2b's initial scan). Service `ai.openclaw.gateway` is a launchd-managed process listening on `127.0.0.1:18789` (HTTP + a web control UI at `/`) plus internal IPC ports `18791`/`18792`. CLI is at `/opt/homebrew/bin/openclaw` (installed via npm). Deploy tree lives at `~/projects/openclaw-deploy`. The gateway exposes a working agent-turn surface: `openclaw agent --json --agent <id> --message <text>` returns a JSON payload with the model's reply — verified live with `--local --message "ping"` which returned a `"pong"` payload after plugin bootstrapping (groq-search, lead-intel, opus-dispatch plugins auto-registered). The earlier "OpenClaw unavailable" conclusion from the 14.2b decision was a false negative — the port probe did not reach 18789. This unblocks Phase 14.2c (real SMS conversational reply) cleanly: the integration shape is "Content Forge shells out to `openclaw agent --json` with the inbound message text + session key, parses the returned payload, sends via Twilio" (or equivalently hits the gateway HTTP/WS surface at 18789 directly — ACP bridge is available). For 11.2 caller (bulk variant generation), the fit is less clean: OpenClaw is turn-oriented, not batch-oriented, so bulk generation either means N agent turns sequenced OR reframing the slice to use `LLM.Anthropic`/`LLM.Gemini` as the bulk generator (already shipped, cheaper and more direct for non-conversational content). Architect recommendation: spec 14.2c next against the live OpenClaw gateway; separately reframe 11.2 caller as an `LLM`-backed slice rather than waiting for an OpenClaw HTTP bulk endpoint that isn't the native shape.
- **2026-04-23:** Architect reframe: tool-call wiring promoted to Phase 16 as "OpenClaw Tool Surface (ecosystem unlock)." Originally scoped as 14.2d — a channel-specific slice inside Phase 14 — which was wrong. The tool surface is agent-scoped, not channel-scoped: tools are registered once with the OpenClaw agent and every channel that talks to that agent (SMS, Telegram, the CLI, anything future) can invoke them. Channel-specific code only handles delivery of the final reply. Pattern precedent is already running in this gateway: the `lead-intel` plugin at `~/.openclaw/plugins/lead-intel/index.js` registers 21 tools and proxies over HTTP to another Elixir app on `localhost:4010`. Content Forge follows the identical shape — a Node.js plugin at `~/.openclaw/plugins/content-forge/index.js` declares the tool schema, and an HTTP endpoint on the existing Phoenix app executes each tool with shared-secret auth. This is arguably the most important remaining piece: the constitution calls Content Forge "the orchestration brain" — tool wiring is how the brain's muscles get connected across every conversational surface the ecosystem has or adds. It is also the extension point other ecosystem apps (contacts4us, Media Forge, the chatbot consultancy) use to register their own plugins against the same gateway, giving the agent a unified ecosystem tool surface. Scope: six sub-slices in Phase 16 — (16.1) plugin scaffold + HTTP tool surface + end-to-end proof on `create_upload_link`, (16.2) read-only tools, (16.3) light-write tools + role-based authorization framework on `ProductPhone.role` (and a new `OperatorIdentity` for CLI-origin calls), (16.4) heavy-write tools with two-turn confirmation on destructive actions, (16.5) unified `ToolInvocationEvent` audit + dashboard, (16.6) escalate-to-human promoted from the 14.5 primitive into a first-class tool the agent invokes on ambiguity. Phase 15 polish still exists and is independent; Phase 16 can ship in parallel or after.
