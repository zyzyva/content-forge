...
- [ ] Image SEO Decorator: Before any HTML or Markdown body containing images leaves Content Forge (blog webhook payload, multi-platform publish, dashboard preview), a single decoration pass enforces three rules. First, every image element has alt text. If the draft author did not supply alt text, the decorator falls back to a human-readable label derived from the image filename slug (for example a stored filename of "red-running-shoes-on-trail.jpg" becomes alt text "Red running shoes on trail"). Generic filenames (matching patterns such as IMG_*, DSC_*, photo*, screenshot*, untitled*, image*, or a bare numeric stem) do not produce a fallback; the image is flagged and the SEO checklist marks it as failing image alt coverage rather than emitting a nonsense alt
- [ ] Image filename as descriptive label: every ProductAsset surfaced through publishing has a "publish filename" — the descriptive slug used when the image is referenced in outbound HTML, Markdown, or platform payloads. The publish filename is derived from the asset's stored display name and tags at upload time (see Feature 11) and is what the SEO decorator uses both as the filename in the URL path and as the source of fallback alt text. Generic stored filenames cause the asset to be flagged "needs description" in the dashboard before it can be attached to a published draft
- [ ] Image hyperlink wrapping: when a draft references an image that has a configured source URL (the canonical product page, the gallery permalink, the original media URL, or the blog post the image belongs to), the decorator wraps the rendered image in an anchor to that URL with rel="noopener" and a title attribute matching the alt text. Images without a configured target URL are emitted unwrapped — the decorator does not invent destinations. Platform publishers that do not support anchored images (most social platforms) ignore the wrapping and use the bare image; only HTML-capable destinations (blog webhook payload, dashboard preview) receive the wrapped form
- [ ] Decorator coverage in the SEO checklist: a new check verifies that every `<img>` and `![alt](url)` in a blog draft has been through the decorator and has non-empty alt text. Drafts where the decorator could not produce alt for any image (because the underlying asset has a generic filename and no author-supplied alt) fail this check and are blocked by the publish gate, surfacing the offending asset IDs in the failure note
...
- [ ] Publish filename normalization: in addition to the original filename, each asset stores a "publish filename" — a descriptive slug used whenever the asset is referenced in outbound content (image URLs, Markdown body, HTML body, social platform payloads). On upload the system derives the publish filename in priority order from (1) a user-supplied descriptive name on the upload form, (2) the asset's tags joined into a slug if no name was given, (3) the asset's description if no tags exist, or (4) the original filename only if it is already descriptive (i.e. not a generic camera or screenshot name). Generic original filenames matching configured patterns (IMG_*, DSC_*, photo*, screenshot*, untitled*, image*, bare numeric stems) without a supplied name, tag, or description cause the asset to be marked "needs description" — the dashboard shows the asset in a warning state and publishing flows refuse to attach it to a draft until a publish filename can be derived
- [ ] Publish filename editing: the asset detail UI lets agency users edit the publish filename directly, with a live preview of how it will appear in alt text (humanized version of the slug) and in the image URL path. Editing the publish filename does not rename the underlying storage object; it only changes how the asset is referenced in published content from that point forward
...
**Confirmation contract (introduced in 16.4):**
- [ ] Heavy-write tools run a two-turn protocol. On the first call (no `confirm` param) the tool returns a confirmation envelope with a human-readable echo phrase, an expiry timestamp, and a tool-provided preview map. The agent reads this back to the user: "this will do X, Y, Z; estimated cost $N; reply with the phrase <echo> to confirm." On the second call the agent includes the echo phrase in a `confirm` param; the tool verifies and executes.
- [ ] Echo phrases are session-scoped, single-use, and bound to a hash of the exact params that originally generated them. Replaying a phrase against different params returns `:confirmation_mismatch`. Replaying an already-consumed phrase returns `:confirmation_not_found`.
- [ ] Pending confirmations expire after a configurable window (default 5 minutes, sourced from `:content_forge, :open_claw_tools, :confirmation_expiry_seconds`). Expired phrases return `:confirmation_expired` on confirm. The tool surface never silently reuses an expired confirmation.
- [ ] The request side is idempotent: a second request with the same `(session_id, tool_name, params_hash)` while a matching pending row is still live returns the existing phrase rather than minting a new one. This means the agent can safely retry the ask if it thinks the user did not hear the first prompt.
- [ ] The controller serializes confirmation envelopes as `200 {"status": "confirmation_required", "echo_phrase": "...", "expires_at": "...", "preview": {...}}`. This is distinct from `200 ok`, `404 unknown_tool`, and `422 error`. Agents key off `status`.
- [ ] The confirmation preview is whatever map the tool provides. Convention: include a `summary` string the agent can read verbatim, plus structured fields (`cost_estimate`, `affected_records`, `diff`) the agent can cite if the user asks a follow-up question before confirming.
